...
export default function TopicDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchTopicAndSubmissions = useCallback(async () => {
    try {
      // お題の詳細を取得
      ...
      const submissionsWithStats =
        submissionsData?.map((submission) => {
          ...
          const averageScores = {
            scalability_score:
              feedbacks.reduce((sum: number, f: Feedback) => sum + f.scalability_score, 0) /
              totalCount,
            security_score:
              feedbacks.reduce((sum: number, f: Feedback) => sum + f.security_score, 0) /
              totalCount,
            performance_score:
              feedbacks.reduce((sum: number, f: Feedback) => sum + f.performance_score, 0) /
              totalCount,
            maintainability_score:
              feedbacks.reduce((sum: number, f: Feedback) => sum + f.maintainability_score, 0) /
              totalCount,
            design_validity_score:
              feedbacks.reduce((sum: number, f: Feedback) => sum + f.design_validity_score, 0) /
              totalCount,
          }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm text-gray-500">目安時間: {topic.estimated_time}</span>
              </div>
            </div>
            <button
              type="button"
        ...
        {/* 投稿一覧 */}
        <div>
          <h2 className="text-2xl font-bold text-gray-900 mb-6">みんなの設計図</h2>
              ...
              {submissions.map((submission) => {
                const totalScore = submission.feedback_summary?.average_scores
                  ? (submission.feedback_summary.average_scores.scalability_score +
                      ...
                      submission.feedback_summary.average_scores.design_validity_score) /
                    5
                  : 0
